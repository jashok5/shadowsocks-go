...
import (
	...
	"github.com/jashok5/shadowsocks-go/internal/protocol/ssr/utils/netx"
	"github.com/jashok5/shadowsocks-go/internal/protocol/ssr/utils/socksproxy"
	"github.com/pkg/errors"
	"github.com/sirupsen/logrus"
)

// ShadowsocksRProxy ShadowsocksProxy is respect shadowsocks proxy service
...
// ShadowsocksRArgs ShadowsocksArgs is ShadowsocksProxy arguments
...
func (ssr *ShadowsocksRProxy) StartTCP() error {
	return ssr.ListenTCP(func(request *network.Request) {
		...
		go func() {
			...
			ssr.handleStageAddr(ssrd.UID, ssrd.RemoteAddr().String(), ssrd.LocalAddr().String(), addr.String())
	...
	err := ssr.ListenUDP(func(request *network.Request) {
		go func() {
			...
			for {
				...
				ssr.handleStageAddr(int(binaryx.LEBytesToUInt32(uid)), addr.String(), ssrd.PacketConn.LocalAddr().String(), remoteAddr.String())
...
func (ssr *ShadowsocksRProxy) handleStageAddr(uid int, client, server, proxyTarget string) {
	if uid == 0 {
		logrus.WithFields(logrus.Fields{
			"uid":         uid,
			"client":      client,
